...
global maxit Memb MembR Isi Fisi1 Fisi2 AFisi AFisi1 AFisi2 T WREF TP;
   ...
   if (0) % old code took M x T arrays of spike times, computed ISI
     ...
     % new form takes raw spike times in seconds (one long single trial)
     isibin = allspikes;
     isicnt = ones(size(isibin));
     redflag = 0;
   end
   ...
   for i = 1:MN
       taur = 0.5 + (0.2 * (1.4^(i+1)));   % time of 0.8 up to 3.2
       tsig = taur/3;   % fixed tightness
       for j = 1:MN
           rate = 3^j;   % 4 up to 1024 Hz
           for ii = 1:MN
               taura = 0.5 + (0.2 * (2.5^(ii)));   % 3.2 up to 110
               tsiga = taura/3;
               for jj = 1:MN
