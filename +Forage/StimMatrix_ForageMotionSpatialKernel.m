function [StimX,StimY] = StimMatrix_ForageMotionSpatialKernel(Exp,GRID,SPclust)
             ...
             for k = 1:KT
                 ...
                 if ~isempty(z)
                    ...
                    for sk = 1:Exp.D{zk}.PR.noiseNum
                       ...
                       sdir = Exp.D{zk}.PR.NoiseHistory(k,4+((sk-1)*4));  % x pos
                       slife = Exp.D{zk}.PR.NoiseHistory(k,5+((sk-1)*4));  % y pos
                       
                       % if (slife > 0)  % time-lock after first flash
