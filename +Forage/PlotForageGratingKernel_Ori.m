function Hart = PlotForageGratingKernel_Ori(StimX,StimY,FileTag)
...
%     Hart: all info we want to pass back
%          one field StimYY = [Nframes,1] - spike counts after removing estimated visual
...
if (1)
  ...
  maxDTA = ceil(80/(1000 * SampRate))-DTA;  % no earlier than 50 ms
else
  %*** for V1
  minDTA = ceil(40/(1000 * SampRate))-DTA;  % no earlier than 50 ms
...
hb = figure; 
if (0)
  subplot('position',[0.1 0.6 0.4 0.3]); plot(tXX,tempker,'k-'); hold on;
...
xlabel('Orientation'); ylabel('Spat Freq');
set(gca,'YTick',MyTick,'YTickLabel',MyTickLabel); title('Gain projection');
end
...
F=vertcat(xo, otune);
F=F';
max1=max(F(:,2));%firing rate for prefered direction
mean1=mean(F(:,2));
column = find(F(:,2)>=(max1));
ORI = xo(column)     % SpatOri of the peak (SHOW)

% Create ORI Index

% If Using ortho 
% if ORI>=90
%    ortho= (ORI-90); %find orthogonal orientation
% else
%    ortho= (ORI+90);
% end
% [row,col]=find(ismember(F,ortho(1,1))); %find row and column in F matrix
% Oorth= F(row,2); %find firing rate for orthogonal

%OI= (Opref-Oorth)/(Opref)
OI = (max1-mean1)/(max1);
%%Robust quantification of orientation selectivity and direction
%%selectivity (Mazurek et al 2014)


% Create Spat Freq vector
xo = SpatFrqs; 
F2=vertcat(xo, stune);
F2=F2';
max2=max(F2(:,2));
mean2=mean(F2(:,2));
column= find(F2(:,2)>=(max2));
SpatFreq = xo(column)   %%show SF

%Create Spatial Frequency Index
%Nonpf=min(F2(1,2)); %% find non prefered
SFI=(max2-mean2)/(max2);


%****** NOW STORE EVERYTHING IN RETURN STRUCT
Hart.stcounts = stcounts;
Hart.stsem = stsem;
Hart.NOri = NOri;
Hart.NSpf = NSpf;
Hart.SpatOris = SpatOris;
Hart.SpatFrqs = SpatFrqs;
Hart.tXX = tXX;
Hart.tempker = tempker;
Hart.sgcounts = sgcounts;
Hart.sgsem = sgsem;
Hart.Ori = ORI;
Hart.OI=OI;
Hart.SFI=SFI;
Hart.SpatFreq = SpatFreq;
Hart.ymean=ymean;
%***
Hart.uu = uu;
Hart.su = su;
