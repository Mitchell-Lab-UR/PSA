function Info = Compute_PFR_Jude4(Info,Exp,H,plotsteps,trialtype)
%******* 
...
%****** if Exp is [], then just perform the plot and return
  ...
  for k = 1:length( Info.Tlist )
    ...
    if (trialtype == 1)  % flashing dots but no motion
        if ( (Exp.D{tr}.P.motionStimulus ~= 1) || (Exp.D{tr}.P.dotSpeed ~= 0) )
            Info.SkipTrial(k) = 1;
            continue;
        end
    end
    ...
    if (trialtype == 3)  % Gabor stimuli but no motion  
        if ( (Exp.D{tr}.P.motionStimulus ~= 0) || (Exp.D{tr}.P.driftSpeed <= 0) )
            Info.SkipTrial(k) = 1;
            continue;
        end
    end
    ...
    if isempty(Info.LocationTarg{tg})
       posx = Info.TargPosX(k,tg);
       posy = Info.TargPosY(k,tg);
       Info.LocationTarg{tg} = [posx,posy];
    end
    %********
