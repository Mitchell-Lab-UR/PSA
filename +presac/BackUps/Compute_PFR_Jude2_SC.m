function Info = Compute_PFR_Jude2_SC(Info,Exp,H,plotsteps)
%******* 
...
function plot_PFR_unity(Info,H,Targ)
   ...
   axis([0 360 0 360]);
   set(gca,'FontSize',16)
   xlabel('True motion direction','Fontsize',16);
   ylabel('PFR motion direction','Fontsize',16);
   %*****
   if (Targ == 1)
      title('Saccade Target Motion','Fontsize',18);
   else
      title('Other Aperture Motion','Fontsize',18);
   end
   %****
...
function plot_PFR_angles(Info,H)  % plot of angle distribution
   ...
   h = legend(sprintf('Target Median Err:%5.1f',mederr),sprintf('Other Median Err:%5.1f',mederr2));
    set(h,'FontSize',8)
