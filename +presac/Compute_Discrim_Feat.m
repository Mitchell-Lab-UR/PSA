function Info = Compute_Discrim_Feat(Info,Exp,Unit,H,FieldName,Event,WinLock,TimeLocks,RTpass)
%******* 
...
%   BigList{2} = Info.IgnAList ; 
%   BigList{3} = Info.IgnBList ;  %all valid trials
%   Rast = cell(1,3);
% for zk = 1:3
...
%       OriSpk{zk}(k) = rate;   
    ...
    % [tt,auc,s_auc] = spikestats.CompCountSlider(tinfo.Rast{zk},PrefOri,NPrefOri,OriVals,OriInd{zk},tinfo.LockWin,tinfo.CountWin,5);
    tinfo.AucTT{zk} = tt;
    tinfo.AucUU{zk} = auc;
    tinfo.AucSU{zk} = s_auc;
    ...
    for kk = 1:length(TimeLocks) 
       ...
       %[uval,sval,ci] = spikestats.Compute_AUC_Circle(OriVals,OriInd{zk},OSpk);
       tinfo.AUC{kk}(zk) = uval; 
       tinfo.AUC_sem{kk}(zk) = sval;
       %*******
    end
