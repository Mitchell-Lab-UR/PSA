...
%****** if Exp is [], then just perform the plot and return
  if isempty(Exp)
    if isfloat(H) && (H > 0) 
       ...
       hh = figure('position',[100 100 700 500]); 
       HA = subplot('position',[0.65 0.1 0.25 0.25]);
       HB = subplot('position',[0.65 0.6 0.25 0.25]);
       HC = subplot('position',[0.12 0.15 0.45 0.7]);
...
%       set(hb,'Markersize',2);
   ...
   axis([0 360 0 360]);
   set(gca,'FontSize',14)
   xlabel('True motion direction','Fontsize',14);
   ylabel('PFR motion direction','Fontsize',14);
   %*****
   if (Targ == 1)
      title('Saccade Target Motion','Fontsize',16);
   else
      title('Other Aperture Motion','Fontsize',16);
   end
   %****
