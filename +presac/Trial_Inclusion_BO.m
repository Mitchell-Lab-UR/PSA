function Info = Trial_Inclusion_BO(Info,Exp,H)
%******* 
%******  function Info = Trial_Inclusion_BO(Info,Exp,PlotIt)
%****
...
%***
  ...
  AmpThresh = 0.40; % fraction of saccade distance of total target ecc
          ...
          if (Exp.D{i}.PR.error == 0) || (Exp.D{i}.PR.error == 4)  % only saccades to an aperture (no fix breaks, etc..) 
               ...
               if ~isempty(slist)
                  ...
                  if (foundsac == 1) 
                       ...
                       %*********
                       if isfield(Exp.D{i}.PR,'dotbandW')
                           CohList = [CohList ; Exp.D{i}.PR.dotbandW];
                       else
                           CohList = [CohList ; 0];
                       end 
