function Info = Trial_Inclusion_SC_FOV(Info,Exp,H,FOVANG,FOVERR)
%******* 
%******  function Info = Trial_Inclusion(Info,Exp,PlotIt,FOVANG,FOVERR)
%****
...
%***          FOVANG - direction of RF (for counting targets as in RF) degs
%***                        for Ellie would be 180 degs, for Milo 112-120 degs
%***          FOVERR - plus or minus degs within FOVANG to count as inside,
%***                       typically would be 50 degs
...
%***
  ...
  %******
  ProbM = zeros(3,3); % Choose vs last item
...
%     Exp.D{i}.PR
        ...
        if ( (Exp.D{i}.PR.error == 0) || (Exp.D{i}.PR.error == 4)) && (Exp.D{i}.P.fixation == 0) 
         ...
         if ~isfield(Exp.D{i}.PR,'target_null') || (Exp.D{i}.PR.target_null ~= 1) % no blank target in RF location
          ...
          if (Exp.D{i}.PR.error == 0) || (Exp.D{i}.PR.error == 4) || (Exp.D{i}.PR.error == 5) % only saccades to an aperture (no fix breaks, etc..) 
               ...
               %*******FOV -- leave everything the same till final moment
               %******* and then categorize based on where RF might be
               ...
               if ~isempty(slist)
                  ...
                  if (foundsac == 1) 
                     ...
                     if ~isempty(FOVANG)  
                       %******* OK, need to decide in FOV trial in RF
                       %*** is there a target that was in the RF, search
                       %*** for a region of visual space by angle
                       TargIn = 0;
                       TargInMotion = NaN;
                       TargInX = NaN;
                       TargInY = NaN;
                       for k = 1:length(Exp.D{i}.PR.targ_x)
                          tgx = Exp.D{i}.PR.targ_x(k);
                          tgy = Exp.D{i}.PR.targ_y(k);
                          dthresh = cos(FOVERR*(pi/180));
                          dot = ((cos(FOVANG*pi/180)*tgx)+(sin(FOVANG*pi/180)*tgy))/norm([tgx,tgy]);
                          if (dot > dthresh)  % a target on this trial was within RF
                              TargInMotion = Exp.D{i}.PR.targ_motion(k);
                              TargIn = k;
                              TargInX = tgx;
                              TargInY = tgy;
                              break;  % nature of task demands only one in RF by def
                          end
                       end
                       if (TargIn == 0) || isnan(TargInMotion)
                          NoRFTarget = NoRFTarget + 1;
                          disp('Dropping trial because no target in RF ');
                          continue;
                       else
                          stimori = TargInMotion;
                          % resort stimuli after so targ=1 is in RF
                          targ_motion = Exp.D{i}.PR.targ_motion';
                          targ_x = Exp.D{i}.PR.targ_x';
                          targ_y = Exp.D{i}.PR.targ_y';
                          %***********
                          if (TargIn ~= 1)
                              disp('Swapping TargIn to keep in pos 1');
                              % input('check'); % happens for Milo
                              % swap targ in with position 1
                              tmp = targ_motion(1);
                              targ_motion(1) = TargInMotion;
                              targ_motion(TargIn) = tmp;
                              %***
                              tmp = targ_x(1);
                              targ_x(1) = TargInX;
                              targ_x(TargIn) = tmp;
                       ...
                       %******** standard if no swaps on foveal case ***
                       targ_motion  = Exp.D{i}.PR.targ_motion';
                       targ_x = Exp.D{i}.PR.targ_x';
                       targ_y = Exp.D{i}.PR.targ_y';
                       %*********** 
                     ...
                     TargMotion = [TargMotion ; targ_motion];
                     TargPosX   = [TargPosX   ; targ_x];
                     TargPosY   = [TargPosY   ; targ_y];
  ...
  disp(sprintf('No RF Targets %d',NoRFTarget));
  ...
  %**** for Rlist, treat it similar to feature gain (compute it there)
  %**** Focused question, do attention effects differ for return saccades
  %******    
  if (NumTargs == 2)  % would be 2 in Flagmo7
     ...
     if (0)  % outdated ... using new code above
       RFtarg = [];
  ...
  %********* Establish resampling lists for future computations  
  ...
  %**********
  Info.MatchAttList = BigList{1};
  Info.MatchIgnAList = BigList{2};
  Info.MatchIgnBList = [];
  %***********
