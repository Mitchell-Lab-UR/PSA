function Info = Compute_PFR_Model2(Info,Exp,H,plotsteps,N)
%******* 
...
%****** if Exp is [], then just perform the plot and return
  if isempty(Exp)
    if isfloat(H) && (H > 0) 
       ...
       hh = figure('position',[100 100 650 490]); 
       HA = subplot('position',[0.72 0.12 0.22 0.29]);
       HB = subplot('position',[0.72 0.62 0.22 0.29]);
       HC = subplot('position',[0.13 0.19 0.45 0.7]);
  ...
  Info.avec = cell(1,NT);  % vectors by attention location
  Info.openvec = [];
  ...
  for k = 1:length( Info.Tlist )
    ...
    if (tg == 1)
       Info.avec{1} = [Info.avec{1} ; svec];
       Info.avec{2} = [Info.avec{2} ; [0,0]];
       if (NT > 2)
           Info.avec{3} = [Info.avec{3} ; [0,0]];
       end
    end
    if (tg == 2)
       Info.avec{1} = [Info.avec{1} ; [0,0]];
       Info.avec{2} = [Info.avec{2} ; svec2];
       if (NT > 2)
           Info.avec{3} = [Info.avec{3} ; [0,0]];
       end    
    end
    ...
    Info.vec{1} = [Info.vec{1} ; tgvec ];
    Info.vec{2} = [Info.vec{2} ; tgvec2];
    if (NT > 2)
       Info.vec{3} = [Info.vec{3} ; tgvec3];
    end
 ...
 %******** Try to use lm to fit PFR trial by trial here ********
  ...
  targ_x = Info.vec{1}(:,1); % Target vectors 
  targ_y = Info.vec{1}(:,2); 
  ...
  if (0)  % location free model
      
      %features = [targ_x,targ_y,other1_x,other1_y,other2_x,other2_y]; %All the vector information for each target
      features_x = [targ_x,other1_x,other2_x];
      features_y = [targ_y,other1_y,other2_y];
  ...
  else  % per locations, with and without saccadic attention term
      
      %********* over-write with more complex attention per location model
      s1_x = Info.svec{1}(:,1);
      s1_y = Info.svec{1}(:,2);
      s2_x = Info.svec{2}(:,1);
      s2_y = Info.svec{2}(:,2);
      ...
      a3_y = Info.avec{3}(:,2);
      %*********
      features_x = [s1_x,s2_x,s3_x,a1_x,a2_x,a3_x];
      features_y = [s1_y,s2_y,s3_y,a1_y,a2_y,a3_y];
  ...
  if (1)
      ...
      ux = mean(beta_x(2:end));
      uy = mean(beta_y(2:end));
      beta_u = sqrt(beta_x.^2 + beta_y.^2);
      beta_x(2:end) = beta_u(2:end);
      beta_y(2:end) = beta_u(2:end);
      % beta_x(5:end) = beta_x(5:end)*2.0;
      %**********
      ...
      mdl_y
      beta_u
      Info.PFR_Model_Weights = beta_u; % 1-s{1},2-s{2},2-s{2},3-a{1},1-a{2},1-a{3}
  end
  
  
  if (1)  % set this to one if you want to convolve model fit with noise residuals
...
%       set(hb,'Markersize',2);
   ...
   axis([0 360 0 360]);
   set(gca,'FontSize',13)
   xlabel('True motion direction','Fontsize',13);
   ylabel('PFR motion direction','Fontsize',13);
   %*****
   if (Targ == 1)
      title('Saccade Target Motion','Fontsize',15);
   else
      title('Other Aperture Motion','Fontsize',15);
   end
   %****
...
function plot_PFR_angles(Info,H)  % plot of angle distribution
   ...
   set(gca,'FontSize',16)
   xlabel('Angle (degs)','Fontsize',16);
   ylabel('Probability','Fontsize',16);
   mederr = Info.PFR_TargErr;
   mederr2 = Info.PFR_DistErr;
   %mederr3 = Info.PFR_ModelErr;
   % title(sprintf('Median Err( Target: %5.1f, Other: %5.1f)',mederr,mederr2));
   title('Example Session','Fontsize',18);
   % h = legend(sprintf('Target Median Err:%5.1f',mederr),sprintf('Other Median Err:%5.1f',mederr2),sprintf('Model Err:%5.1f',mederr3);
%    set(h,'FontSize',8)
%   set(h,'Position',[0.535 0.8 0.08 0.07])
